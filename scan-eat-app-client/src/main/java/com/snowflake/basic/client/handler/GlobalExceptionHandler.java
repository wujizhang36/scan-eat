...
public class GlobalExceptionHandler extends SaTokenExceptionHandler {
    ...
    public ResponseEntity<ApiResultModel<String>> handleCustomException(BusinessException ex) {
        ...
        ApiResultModel<String> apiResult = ApiResultHelper.error(ex.getErrorCode(), ex.getErrorMsg());
    ...
    public ResponseEntity<ApiResultModel<String>> handleSystemException(Exception ex) {
        log.error("{}", ex);
        // 创建 ApiResultModel 对象
        String msg = messageSource.getMessage(ExceptionMessageEnum.ERROR_000000.getLanguageKey(), null, new Locale("zh", "CN"));
        msg = msg + " " + messageSource.getMessage(ExceptionMessageEnum.ERROR_000000.getLanguageKey(), null, new Locale("en", "US"));
        ApiResultModel<String> apiResult = ApiResultHelper.error(ExceptionMessageEnum.ERROR_000000.getErrorCode(), msg);
        // 返回带有错误消息和状态码的 ResponseEntity
        return new ResponseEntity<>(apiResult, HttpStatus.INTERNAL_SERVER_ERROR);
    }
